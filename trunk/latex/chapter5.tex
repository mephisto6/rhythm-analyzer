...
A két dimenzióra leképzõ általános algoritmus lépései a következõk \cite{Wickelmaier}:

\begin{enumerate}
\item Rendezzük el véletlenszerûen az elemeket a síkon.
\item Számoljuk ki a pontok közötti $d_{ij}$ távolságokat.
\item A távolságmátrix elemeit egy monoton $f$ transzformáció segítségével skálázzuk.
\item Minimalizáljuk az alábbi S stresszfüggvényt a pontok újrarendezésével.
\begin{align}
S=\sum_{i=1}^D \sum_{j=1}^D (f(p_{ij})-d_{ij})^2
\end{align}
\item Ha nem elég kicsi az eredmény, folytassuk a $2$-es pontnál. 
\end{enumerate}

Az esetünkben alkalmazott konkrét, gradiens alapú újrarendezõ algoritmus és az $f$ transzformáció részletezése megtalálható a forrásjegyzékben \cite{MDS}.
...
Az \ref{fig:cluster1MDS} ábrán látható a $30$ dalból álló elsõ dallamvonal szerinti osztály Hamming-távolság alapján számított MDS-térképe.
...
Vizsgáljuk meg, hogy a klaszterezések által adott eredmények mennyire tükrözõdnek a SplitsTree által rajzolt fákon! Ehhez használjuk az ötödik dallam szerinti csoport $33$ elemét. Ekkor az alapértelmezett $K$ az \ref{eq:defk} képlet szerint $\sqrt{\frac{33}{2}}\approx 4$.
...
Az \ref{fig:cluster5average} ábra mutatja, hogy az average linkage algoritmus ennél hatékonyabb, hiszen használatával két egymástól jól elkülönülõ, nagy klasztert és két, egy-egy outlier pontból álló klasztert kapunk.
...
\subsubsection{A klaszterek belsõ szekezete}
...
Ha vetünk egy pillantást a klaszterek eleminek kottáira is, kiderül, hogy az \ref{fig:cluster5furt1} ábrán látható dalok közös tulajdonsága a ti-táj-táá zárlat (az ezekhez tartozó dalok a fenti complete linkage szerinti \ref{fig:cluster5complete} ábra kékkel és zölddel jelölt elemei).
...
Tanulságos lehet megfigyelni, hogy különbözõ módszerrel számított távolságmátrix alapján rajzolt ábrák klaszterezései is hasonló eredményt mutatnak-e. Ehhez vizsgáljuk meg az intervallum-vektorok Euklideszi-távolsága alapján számított távolságmátrixból épült fát és rajta az average linkage szerinti színezést (\ref{fig:cluster5euc_average} ábra). A színeket úgy választottam meg, hogy látható legyen a klaszterezés eredményeinek hasonlósága. 
